...
TRAINING_ARGS = CPOConfig(
    ...
    beta=0.1,  # Temperature parameter for CPO loss - PLEASE CONFIRM IF THIS VALUE IS OK
    cpo_alpha=0.0,  # Alpha parameter for CPO - PLEASE CONFIRM IF THIS VALUE IS OK
    loss_type="sigmoid",  # CPO supports different loss types: "sigmoid", "hinge", "ipo", "simpo" - PLEASE CONFIRM
    label_smoothing=0.1,
    ...
    output_dir="/root/autodl-tmp/selfmodel/CPO_trained",
    # The trained model will be pushed to this repository on the Hub
    hub_model_id="Eehan/Qwen2.5-1.5B-cpo-flip-hh",
    push_to_hub=True,
    save_strategy="no",
    report_to=["wandb"],
)
...
def train_model(model_args: ModelConfig, script_args: ScriptArguments, training_args: CPOConfig):
    """
    Loads a base model, trains it using CPOTrainer, and pushes it to the Hub.
    """
    ...
    # Remove columns that are not needed for CPO training
