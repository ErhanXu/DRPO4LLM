...
TRAINING_ARGS = DrDPOConfig(
    ...
    output_dir="/root/autodl-tmp/selfmodel/dr_dpo_trained",
    # The trained model will be pushed to this repository on the Hub
    hub_model_id="Eehan/Qwen2.5-1.5B-dr_dpo-flip-hh",
    push_to_hub=True,
    save_strategy="no",
    report_to=["wandb"],
)
...
def train_model(model_args: ModelConfig, script_args: ScriptArguments, training_args: dr_dpoConfig):
    ...
    # Remove columns that are not needed for dr_dpo training
