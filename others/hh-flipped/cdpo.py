...
# cDPO training configuration
TRAINING_ARGS = DPOConfig(
    # cDPO-specific parameters
    beta=0.1,  # KL regularization coefficient - PLEASE CONFIRM IF THIS VALUE IS OK
    label_smoothing=0.1,  # cDPO typically doesn't use label smoothing - PLEASE CONFIRM
    ...
    output_dir="/root/autodl-tmp/selfmodel/cDPO_trained",
    # The trained model will be pushed to this repository on the Hub
    hub_model_id="Eehan/Qwen2.5-1.5B-cDPO-flip-hh",
    push_to_hub=True,
    save_strategy="no",
    report_to=["wandb"],
)
...
def train_model(model_args: ModelConfig, script_args: ScriptArguments, training_args: DPOConfig):
    """
    Loads a base model, trains it using DPOTrainer with conservative DPO loss, and pushes it to the Hub.
    """
