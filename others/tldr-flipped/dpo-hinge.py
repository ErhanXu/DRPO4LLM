...
# DPO-hinge training configuration
TRAINING_ARGS = DPOConfig(
    # DPO-hinge specific parameters
    loss_type="hinge",  # Using hinge loss for DPO
    label_smoothing=0.1,  # Label smoothing for DPO-hinge
    ...
    output_dir="/root/autodl-tmp/selfmodel/DPO_hinge_tldr",
    # The trained model will be pushed to this repository on the Hub
    hub_model_id="Eehan/pythia-1b-dpo-hinge-tldr",
    push_to_hub=True,
    save_strategy="no",
    report_to=["wandb"],
)
...
def train_model(model_args: ModelConfig, script_args: ScriptArguments, training_args: DPOConfig):
    """
    Loads a base model, trains it using DPOTrainer with hinge loss, and pushes it to the Hub.
    """
    logging.info("--- Starting DPO-Hinge Model Training ---")
    ...
    logging.info("Loading reference model for DPO-hinge")
