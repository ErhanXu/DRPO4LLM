...
TRAINING_ARGS = CPOConfig(
    ...
    beta=0.1,  # Temperature parameter for CPO loss
    cpo_alpha=0.0,  # Alpha parameter for CPO
    loss_type="sigmoid",  # CPO supports different loss types: "sigmoid", "hinge", "ipo", "simpo"
    ...
    output_dir="/root/autodl-tmp/selfmodel/CPO_tldr_trained",
    # The trained model will be pushed to this repository on the Hub
    hub_model_id="Eehan/pythia-1b-cpo-flip-tldr",
    push_to_hub=True,
    save_strategy="no",
    report_to=["wandb"],
)
...
def train_model(model_args: ModelConfig, script_args: ScriptArguments, training_args: CPOConfig):
    ...
    """
    logging.info("--- Starting CPO Model Training ---")
