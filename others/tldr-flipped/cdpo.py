...
# cDPO training configuration (DPO with label smoothing)
TRAINING_ARGS = DPOConfig(
    # cDPO specific parameters
    loss_type="sigmoid",  # Standard DPO loss with label smoothing
    label_smoothing=0.1,  # cDPO uses label smoothing
    ...
    output_dir="/root/autodl-tmp/selfmodel/cDPO_tldr_trained",
    # The trained model will be pushed to this repository on the Hub
    hub_model_id="Eehan/pythia-1b-cdpo-flip-tldr",
    push_to_hub=True,
    save_strategy="no",
    report_to=["wandb"],
)
...
def train_model(model_args: ModelConfig, script_args: ScriptArguments, training_args: DPOConfig):
    """
    Loads a base model, trains it using DPOTrainer with label smoothing (cDPO), and pushes it to the Hub.
    """
    logging.info("--- Starting cDPO Model Training ---")
    ...
    # Load reference model for cDPO (using same base model)
    logging.info("Loading reference model for cDPO")
    ...
    trainer = DPOTrainer(
        model=model,
        ref_model=ref_model,  # cDPO requires a reference model
        args=training_args,
