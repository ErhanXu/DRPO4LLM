...
# RobustDPO (rDPO) training configuration
TRAINING_ARGS = DPOConfig(
    # RobustDPO-specific parameters
    loss_type="robust",  # Using robust loss for DPO
    beta=0.1,  # KL regularization coefficient
    label_smoothing=0.1,  # RobustDPO typically doesn't use label smoothing
    ...
    output_dir="/root/autodl-tmp/selfmodel/rDPO_tldr_trained",
    # The trained model will be pushed to this repository on the Hub
    hub_model_id="Eehan/pythia-1b-rdpo-flip-tldr",
    push_to_hub=True,
    save_strategy="no",
    report_to=["wandb"],
)
...
def train_model(model_args: ModelConfig, script_args: ScriptArguments, training_args: DPOConfig):
    """
    Loads a base model, trains it using DPOTrainer with robust loss, and pushes it to the Hub.
    """
    logging.info("--- Starting RobustDPO (rDPO) Model Training ---")
    ...
    # Load reference model for RobustDPO (using same base model)
    logging.info("Loading reference model for RobustDPO")
    ...
    trainer = DPOTrainer(
        model=model,
        ref_model=ref_model,  # RobustDPO requires a reference model
        args=training_args,
